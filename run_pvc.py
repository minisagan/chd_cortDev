...
# [SM]: https://github.com/ralidimitrova/neo_cortexDev/blob/main/pvc/run_pvc.py
# [SM]: added mask for negative dwi metric values (s.t. neg vals are included and removed in metric dilate)
...
def _pvc_main_loop(func, pve, mask_coord, x_crd, y_crd, z_crd):
    ...
    # Check for negative GM estimates
    neg_indices = np.where(b[:, 0] < 0)[0]  # Find indices where GM estimate is negative
    
    if len(neg_indices) > 0:
       print(f"Negative values detected in {len(neg_indices)} voxels!")	
...
def _metric_dilate_and_mask(func_gii, surf_gii, roi_gii, surf_mask, distance=10):
    wb_cmd = os.getenv('DHCP_WBCMD', 'wb_command')
    # the roi_gii contains all the negative vertices and masks them out 
    run([wb_cmd, '-metric-math', 
         'x <= 0', roi_gii, 
         '-var', 'x', func_gii])
...
parser.add_argument('dwi_image_native',  help='full path to dwi_image in native dwi space, note, this is still the metric dwi file')
...
parser.add_argument('bad_vertex_rois', help='path to store bad vertex rois, these are created to mask out negative values from FA and MD maps etc (holes)' )
...
for hemi in ['left', 'right']:
        ...
        _metric_dilate_and_mask(
            func_gii='{}/{}_{}_{}_dwi.func.gii'.format(args.dwi_surf_dir, args.subj, hemi, args.dwi_metric),
            roi_gii='{}/{}_{}_{}_bad_vertex_roi.func.gii'.format(args.bad_vertex_rois, args.subj, hemi, args.dwi_metric),
...
for hemi in ['left', 'right']:
    ...
    _metric_resample(
        ...
        current_sphere = '{}/{}_hemi-{}_from-native_to-dhcpSym40_dens-32k_mode-sphere.reg40.surf.gii'.format(args.surf_transf, args.subj, hemi), 
        new_sphere = '/data1/dHCP/dhcpSym_template/week-40_hemi-{}_space-dhcpSym_dens-32k_sphere.surf.gii'.format(hemi), 
